var sidebar = {
	enableSidebarButtons:function(){
		...
		if (baseSelected){			
			if(game.currentLevel.requirements.buildings.indexOf('starport')>-1  && !game.deployBuilding && cashBalance>=buildings.list["starport"].cost){
				$("#starportbutton").removeAttr("disabled");
			}
			if(game.currentLevel.requirements.buildings.indexOf('ground-turret')>-1 && !game.deployBuilding && cashBalance>=buildings.list["ground-turret"].cost){
